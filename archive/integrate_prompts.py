...
PROMPT_ORCHESTRATEUR = """Tu coordonnes l'analyse complète d'un plan électrique BTP en déléguant à 5 agents spécialisés.
...
PROMPT_INFOS_GENERALES = """Tu extrais les métadonnées administratives et techniques d'un plan électrique BTP.
...
PROMPT_COURANTS_FORTS = """Tu analyses exhaustivement les installations électriques courants forts (230V/400V) d'un plan BTP selon la norme NFC 15-100.
...
PROMPT_COURANTS_FAIBLES = """Tu analyses exhaustivement les installations courants faibles (télécom, réseau, sécurité) d'un plan BTP.
...
PROMPT_SECURITE = """Tu vérifies la conformité électrique et identifies les points de sécurité d'un plan BTP selon la norme NFC 15-100.
...
PROMPT_AUTOMATISMES = """Tu identifies tous les équipements automatisés et domotiques d'un plan électrique BTP.
...
for node in workflow['nodes']:
    if node['id'] == 'orchestrateur':
        node['parameters']['systemMessage'] = PROMPT_ORCHESTRATEUR
    elif node['id'] == 'tool-infos-generales':
        node['parameters']['systemMessage'] = PROMPT_INFOS_GENERALES
    elif node['id'] == 'tool-courants-forts':
        node['parameters']['systemMessage'] = PROMPT_COURANTS_FORTS
    elif node['id'] == 'tool-courants-faibles':
        node['parameters']['systemMessage'] = PROMPT_COURANTS_FAIBLES
    elif node['id'] == 'tool-securite':
        node['parameters']['systemMessage'] = PROMPT_SECURITE
    elif node['id'] == 'tool-automatismes':
        node['parameters']['systemMessage'] = PROMPT_AUTOMATISMES

with open('workflow-updated.json', 'w', encoding='utf-8') as f:
    json.dump(workflow, f, indent=2, ensure_ascii=False)

print("✅ Prompts intégrés avec succès dans workflow-updated.json")
print("\nNœuds mis à jour :")
print("- Orchestrateur")
print("- agent_infos_generales")
print("- agent_courants_forts")
print("- agent_courants_faibles")
print("- agent_securite")
print("- agent_automatismes")
